...
- **`chat_ui.py`** — Colors, spinners, Tab selector, markdown rendering, welcome boxes
- **`chat_commands.py`** — Command extraction from ```bash blocks, execution, placeholder resolution, trusted commands
- **`chat_server.py`** — Server communication, SSE streaming, health checks
...
- **`backend.py`** — Backend abstraction: cursor CLI, cursor HTTP, claude SDK, claude CLI (subscription)
- **`chat_history.py`** — Chat session persistence: auto-save to `~/.code-agents/chat_history/`
- **`token_tracker.py`** — Token usage tracking: per message/session/day/month/year, CSV export
- **`stream.py`** — SSE streaming with ToolUse/ToolResult logging, `build_prompt()` for multi-turn
- **`connection_validator.py`** — Async backend connection validation (cursor/claude)
...
- **Interactive chat**: `code-agents chat` → numbered menu → REPL with streaming. `/agent` switches permanently. `/<agent> <prompt>` delegates one-shot to another agent. Tab-completion for commands and agent names.
- **3 Backends**: `"cursor"` (default), `"claude"` (Anthropic API), `"claude-cli"` (Claude subscription, no API key). Set `CODE_AGENTS_BACKEND=claude-cli` globally to override all agents.
...
- **Agent rules**: Two-tier rules injected into system prompts. Global (`~/.code-agents/rules/`) + project (`{repo}/.code-agents/rules/`). `_global.md` = all agents, `{agent-name}.md` = specific agent. Auto-refresh on every message.
- **Token tracking**: Per message/session/day/month/year with CSV export at `~/.code-agents/token_usage.csv`. `/tokens` command in chat.
- **Agentic loop**: Agent outputs ```bash → Tab selector approval → executes locally → output fed back to agent automatically.
