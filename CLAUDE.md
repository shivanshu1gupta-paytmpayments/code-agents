...
- **`code_agents/cli.py`** — Unified CLI entry point: 18 commands (init, migrate, start, chat, shutdown, status, doctor, config, logs, branches, diff, test, review, pipeline, agents, curls, setup, version, help)
...
- 151 tests in `tests/` — `poetry run pytest`
...
- `test_cli.py` (24): server URL, help completeness (all 18 cmds, slash cmds, agents), version, doctor, config, curls, dispatcher
