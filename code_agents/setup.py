...
def write_env_file(env_vars: dict[str, str]) -> None:
    """Write .env file with section grouping."""
    env_path = Path(".env")

    if env_path.exists():
        existing = parse_env_file(env_path)
        existing_count = len(existing)
        print(f"  Found existing .env with {existing_count} configured variables.")
        ...
        if choice == "c":
            print(yellow("  Skipped — .env not modified."))
            return
        if choice == "b":
            ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            backup = Path(f".env.backup.{ts}")
    ...
    lines = ["# Generated by code-agents-setup", ""]
    ...
    # Any remaining keys not in a section
    ...
    count = len(env_vars)
    print(green(f"  ✓ .env written ({count} variables, permissions: 600)"))
    print()
